...
#----------------------------
# A global object that holds the
# word-to-subreddits dictionary
# with each words frequency.
#----------------------------
...
def run_slang_analyser(subnames,
                       word_to_subs_global_path="word_frequencies_to_subreddit.pkl",
                       total_freq_path="total_freq.pkl",
                       subs_to_num_path="subs_to_num.pkl", 
                       save_name='temporary2', save_file=False, pmi=True):
    ...
    Args:
        subnames: a list of subreddit names, of which directories should be accessed and their lemma 
            dictionaries should be used to extract high affinity terms, relative to other subreddits.
    ...
    for i_position in range(len(subnames)):
        if pmi:
            affinity_dic_list.append(pool.apply(bayesian_probability, args=(subnames, i_position, total_freq_path, subs_to_num_path)))
        else:
...
def bayesian_probability(subnames, target_sub_index, total_freq_path="total_freq.pkl", subs_to_num_path="subs_to_num.pkl"):
    """Computes the affinity values of terms in a subreddit.
    ...
    Args:
        ...
        total_freq: a dictionary that contains the total occurrences for each subreddit.
        subs_to_num: a dictionary that maps a subreddit to number index
        target_sub_index: this is the index of the subreddit within subnames for which 
                affinity terms are being calculated. 
    ...
    for word in target_sub_word_freq_dictionary:
        ...
        if coeff <= 1:
            ...
            coeff = 1
        num = coeff*((target_sub_word_freq_dictionary[word]/total_freq[target_dictionary_index])/a)
        affinity_value_dictionary[word] = num
    return affinity_value_dictionary


def load_affinity_terms(sub):
    """Loads affinity terms from a file path provided.""" 
